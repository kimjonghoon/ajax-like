...
$(document).ready(function () {
	//♥(like), ♡(unlike)	
	var url = '/like/';
	var $articleNo = $('article').attr('title');
	url += $articleNo;
	$.getJSON(url, function (data) {
		console.log(data);
	...
	$('#like').bind('click', function() {
		...
		var check = $('#like').text();
		if (check === '♥') {
			var url = $('#deleteLikeForm').attr('action');
			url += $articleNo;
			var dataToBeSent = $('#deleteLikeForm').serialize();
			$.ajax({
				url: url,
				type: 'POST',
				data: dataToBeSent,
			...
			var url = $('#addLikeForm').attr('action');
			$('#addLikeForm input[name*=articleNo]').val($articleNo);
			var dataToBeSent = $('#addLikeForm').serialize();
			$.ajax({
			   url: url,
			   type: 'POST',
			   data: dataToBeSent,
...
<body>
	...
	<div id="wrap">
		...
		<article title="1111">
			...
			TSCHICK Trailer (2016) <span id="like">♡</span>
		</article>
	...
	<div id="form-group" style="display: none">
		...
		<sf:form id="deleteLikeForm" action="/like/" method="delete">
			<input type="hidden" name="_method" value="DELETE" />
		</sf:form>
		<sf:form id="addLikeForm" action="/like" method="post">
